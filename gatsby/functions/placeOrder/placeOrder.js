...
// make a function that generate email
function generateOrderEmail({ order, total }) {
  return `<div>
    ...
    <p>Your total is <strong>$${total}</strong> due at puckup.</p>
    <style>
      ul {
        list-style: none;
      }
...
exports.handler = async (event, context) => {
  ...
  if (!body.order.length) {
    return {
      statusCode: 400,
      body: JSON.stringify({
        message: `Why wouild you order nothing?!`,
      }),
    };
  }
  ...
  // send the success or errror message
